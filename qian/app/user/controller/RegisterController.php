...
class RegisterController extends HomeBaseController
{
    ...
    public function group()
    {
        ...
        $find =Db::name('score')->where('uid',$uid)->find();
        if ($find) {
            if($this->request->isPost()){
                $like =Request::instance()->param();
                $like =array_keys($like);
                //dump($like);
                foreach ($like as $key => $value) {
                    $user = [$like[$key]=>'1','uid'=>$uid];
                    Db::name('score')->update($user);
                }
                $this->redirect('user/register/tj');
            }
        }else{
            $user = ['uid'=>$uid];
    ...
    public function doRegister()
    {
        if ($this->request->isPost()) {
            ...
            switch ($log) {
                case 0:
                    $this->success('注册成功',url('user/register/group'));
                    break;
                case 1:
                    $this->error("您的账户已注册过");
                    break;
