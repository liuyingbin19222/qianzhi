...
class ProfileController extends UserBaseController
{
    ...
    public function center()
    {
        ...
        $this->assign($user);
        $tag = Db::name('portal_tag')->where('uid',$user['id'])->distinct(true)->order('create_time', 'desc')->column('name');

        //     dump($tag);
        $score = Db::name('score')->where('uid',$user['id'])->find();
        $this->assign($user);
        $this->assign('score',$score);
        $this->assign('tag',$tag);
        if (Request::instance()->isMobile()) {
            return $this->fetch('centermoble');
            //调用移动模板
